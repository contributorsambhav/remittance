...
// Contract ABI - only the functions we need for sending money
const REMITTANCE_ABI = parseAbi([
  // Core function for sending remittance
...
export function SendMoneyForm() {
  ...
  const handleConfirmSend = async () => {
    ...
    try {
      ...
      // Check if user has enough remaining limit
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Validation is done in the confirmation modal trigger
  }
  ...
  // Format remaining limit for display
  const remainingLimitFormatted = remainingLimit 
    ? parseFloat(formatEther(remainingLimit)).toFixed(4)
    : "0"
      ...
      <CardContent>
        ...
        <div className="mb-4 p-3 bg-muted/50 rounded-lg space-y-2">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium">
              {hasTierError ? 'Error loading' : getTierLabel(userTier || 0)}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Available to send today:</span>
            <span className="font-medium text-blue-600">
              {hasLimitError ? 'Error loading' : `${remainingLimitFormatted} ETH`}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Account Status:</span>
            <span className={canSend ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>
              {canSend ? 'Ready to Send' : 'Cannot Send'}
            </span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {amount && remainingLimit && parseEther(amount) > remainingLimit && (
              <p className="text-sm text-red-600">
                Amount exceeds daily limit (Available: {remainingLimitFormatted} ETH)
              </p>
            )}
          </div>
          ...
          <ConfirmationModal
            ...
            description={
              <div className="space-y-2">
                <p>Are you sure you want to send <strong>{amount} ETH</strong> to:</p>
                <p className="font-mono text-sm break-all bg-muted p-2 rounded">{recipient}</p>
                <p className="text-sm text-muted-foreground">
                  This transaction cannot be reversed once confirmed on the blockchain.
                </p>
              </div>
            }
