...
import { Shield, Clock, CheckCircle, XCircle, AlertCircle, RefreshCw, AlertTriangle } from "lucide-react"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {isLoading ? "Loading..." : 
               hasErrorUserInfo ? "Unable to load status message" :
               kycStatus ? getStatusMessage(kycStatus) : "Unable to determine status"}
            </p>
          </div>
        </div>
        ...
        {kycStatus === "APPROVED" && !isLoading && !hasErrorUserInfo && (
          <div className="bg-green-50 border border-green-200 p-3 rounded-lg">
            ...
            <div className="text-sm text-green-700 space-y-1">
              ...
              <p>
                • Daily Limit: <span className="font-medium">
                  {dailyLimit ? `${parseFloat(dailyLimit).toFixed(4)} ETH` : "Unknown"}
                </span>
              </p>
              <p>• Full platform access enabled</p>
              {kycTimestamp && (
                <p>• Approved on: <span className="font-medium">{kycTimestamp.toLocaleDateString()}</span></p>
              )}
            </div>
          </div>
        )}
        ...
        {kycStatus === "NONE" && !isLoading && !hasErrorUserInfo && (
          <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg">
            ...
            <p className="text-sm text-blue-700 mb-3">
              Complete your KYC verification to access all platform features and increase your transaction limits.
            </p>
