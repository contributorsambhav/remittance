    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{isBalanceLoading ? <div className="h-8 w-20 bg-muted animate-pulse rounded" /> : hasBalanceError ? <span className="text-sm text-red-600">Error loading</span> : `${balanceFormatted} ETH`}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{isTierLimitLoading || hasTierLimitError ? <div className="h-8 w-20 bg-muted animate-pulse rounded" /> : `${parseFloat(dailyLimitFromContract).toLocaleString()} ETH`}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{isDataLoading ? <div className="h-8 w-20 bg-muted animate-pulse rounded" /> : `${parseFloat(usedToday).toLocaleString()} ETH`}</div>
            <Progress value={Math.min(100, Math.max(0, dailyUsagePercentage))} className="mt-2" />
            <p className="text-xs text-muted-foreground mt-1">{parseFloat(remainingLimitFormatted).toLocaleString()} ETH remaining</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-bold">{isDataLoading ? 'Loading...' : `${parseFloat(dailyLimitFromContract).toLocaleString()} ETH`}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Used Today</span>
              <span className="text-sm">{isDataLoading ? 'Loading...' : `${parseFloat(usedToday).toLocaleString()} ETH`}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Remaining</span>
              <span className="text-sm font-medium text-green-600">{isDataLoading ? 'Loading...' : `${parseFloat(remainingLimitFormatted).toLocaleString()} ETH`}</span>
            </div>
            {!isDataLoading && (
              <>
