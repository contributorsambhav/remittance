...
// Complete Contract ABI for user data functions
...
export function ProfileTab() {
  ...
  // Contract reads for user information
  const { 
    data: userInfo, 
  ...
  useEffect(() => {
    ...
    userInfo, loadingUserInfo, hasErrorUserInfo, errorUserInfo,
    kycRequest, loadingKYCRequest, hasErrorKYCRequest, errorKYCRequest
  ])
  ...
  const remainingLimit = userInfo && dailyLimit && todayUsed 
    ? (parseFloat(dailyLimit) - parseFloat(todayUsed)).toFixed(4)
    : undefined
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {isLoading ? (
                ...
                renderErrorState("Error loading tier")
              ) : tierName ? (
                <Badge className={getTierColor(tierName)}>{tierName}</Badge>
              ) : (
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {isLoading ? (
                ...
                <Badge variant={isWhitelistedUser ? "outline" : "destructive"} 
                      className={isWhitelistedUser ? "text-green-600 border-green-200" : ""}>
            ...
            <div className="flex items-center justify-between">
              ...
              {isLoading ? (
                ...
                <Badge variant={isBlacklistedUser || isFrozenUser ? "destructive" : "outline"} 
                      className={!(isBlacklistedUser || isFrozenUser) ? "text-green-600 border-green-200" : ""}>
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                <p>Current Tier: {
                  isLoading ? "Loading..." :
                  hasErrorUserInfo ? "Error" :
                  tierName || "Unknown"
                }</p>
                <p>Daily Limit: {
                  isLoading ? "Loading..." :
                  hasErrorUserInfo ? "Error" :
                  dailyLimit ? `${parseFloat(dailyLimit).toFixed(4)} ETH` : "Unknown"
                }</p>
                <p>Used Today: {
                  isLoading ? "Loading..." :
                  hasErrorUserInfo ? "Error" :
                  todayUsed ? `${parseFloat(todayUsed).toFixed(4)} ETH` : "Unknown"
                }</p>
                <p>Remaining: {
                  isLoading ? "Loading..." :
                  hasErrorUserInfo ? "Error" :
                  remainingLimit ? `${remainingLimit} ETH` : "Unknown"
                }</p>
              </div>
            </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground space-y-1">
                    ...
                    {kycDocumentHash && (
                      <p>Document Hash: {kycDocumentHash.slice(0, 20)}...</p>
                    )}
                  </div>
                </div>
              </>
