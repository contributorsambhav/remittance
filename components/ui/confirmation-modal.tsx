...
import { AlertTriangle, Shield, UserX, Pause, Play, Crown } from "lucide-react"
...
// Specialized confirmation modals for common actions
export function EmergencyConfirmationModal({
  ...
  return (
    <ConfirmationModal
      trigger={trigger}
      title={title}
      description={`${description} This action cannot be undone and requires immediate attention.`}
      confirmText="Execute Emergency Action"
      onConfirm={onConfirm}
      variant="destructive"
...
export function UserActionConfirmationModal({
  ...
  trigger: React.ReactNode
  action: "freeze" | "unfreeze" | "whitelist" | "remove-whitelist" | "change-tier"
  userAddress: string
  onConfirm: () => void
}) {
